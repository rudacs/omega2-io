...
Omega2.prototype.pinEnd = function (pin) {
	if (pin.temporal) {
		pin.temporal.kill('SIGINT');
		pin.temporal = null;
	}
};
...
Omega2.prototype.digitalRead = function( pinIndex, handler ) {
	...
	var cp = CP.spawn( 'gpio-test', [ pinIndex ]);
	cp.on( 'error', function( err ) {
		return console.error( 'Error reading Omega2 pin ' + pinIndex, err );
	});
	cp.stdout.on( 'data', function( data ) {
		...
		for (var i = 0; i < data.length; i++) {
			var val = data[i].split(toHexString(pinIndex));

			if (val.length == 2) {
				val = parseInt(val[1].trim());
				val = val === 1 || val === 0 ? val : pin.value;
